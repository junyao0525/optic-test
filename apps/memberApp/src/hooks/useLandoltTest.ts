...
export const useLandoltTest = () => {
  ...
  const debouncedProcessSwipe = useDebounce(500); // 500ms debounce delay

  const processSwipe = useCallback(
    (swipeDirection: Direction) => {
      if (isProcessing) {
        console.log('⚠️ Still processing previous swipe');
        return;
      }
    ...
    [step, direction, testState, getRandomDirection, handleIncorrectAnswer, resetTestState, debouncedProcessSwipe, isProcessing],
  );
