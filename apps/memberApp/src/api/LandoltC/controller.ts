...
// Interface for Landolt test results based on your database schema
...
export const LandoltController = {
  /**
   * Insert a new Landolt test result
   */
  insertTestResult: async (
    testResult: LandoltTestResult
  ): Promise<{ success: true; data: LandoltTestResultResponse } | { success: false; message: string }> => {
    try {
      const { data, error } = await supabase
        .from('LandoltCResult') // Replace with your actual table name
        .insert([testResult])
        .select()
        .single();
      ...
      return {
        success: true,
        data: data as LandoltTestResultResponse
      };
    } catch (err: any) {
